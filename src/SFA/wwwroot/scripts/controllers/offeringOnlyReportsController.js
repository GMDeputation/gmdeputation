app.controller('offeringOnlyReportsController', function ($scope, $window, $mdDialog, reportsService, districtService, sectionService, churchService, serviceTypeService) {
    $scope.reportParams = {};
    ...
    $scope.search = function () {
        reportsService.getOfferingOnlyReportData($scope.reportParams).then(function (resp) {
    ...
    $scope.generateExcell = function () {
        if ($scope.searchDatas.length === 0) {
            ...
            reportsService.generateOfferingOnlyReport($scope.searchDatas).then(processSuccess, processError);
        }
    };
    ...
    $scope.generatePdf = function () {
        if ($scope.searchDatas.length === 0) {
            ...
            reportsService.generateOfferingOnlyReportPdf($scope.searchDatas).then(processSuccess, processError);
        }
    };
    ...
    $scope.generateWord = function () {
        if ($scope.searchDatas.length === 0) {
            ...
            reportsService.generateOfferingOnlyReportWord($scope.searchDatas).then(processSuccess, processError);
        }
    };
    ...
    function init() {
        ...
        serviceTypeService.getAll().then(function (resp) {
            ...
            angular.forEach($scope.serviceTypes, function (value) {
                if (value.name == "Offering Only") {
                    $scope.reportParams.serviceTypeId = value.id;
                }
            });
        });
    }
