app.controller('churchServiceCountReportController', function ($scope, $window, $mdDialog, reportsService, districtService, sectionService, churchService) {
    $scope.reportParams = {};
    ...
    $scope.search = function () {
        reportsService.getChurchServiceCountReportData($scope.reportParams).then(function (resp) {
    ...
    $scope.generateExcell = function () {
        if ($scope.searchDatas.length === 0) {
            ...
            reportsService.generateChurchServiceCountReport($scope.searchDatas).then(processSuccess, processError);
        }
    };
    ...
    $scope.generatePdf = function () {
        if ($scope.searchDatas.length === 0) {
            ...
            reportsService.generateChurchServiceCountReportPdf($scope.searchDatas).then(processSuccess, processError);
        }
    };
    ...
    $scope.generateWord = function () {
        if ($scope.searchDatas.length === 0) {
            ...
            reportsService.generateChurchServiceCountReportWord($scope.searchDatas).then(processSuccess, processError);
        }
    };
